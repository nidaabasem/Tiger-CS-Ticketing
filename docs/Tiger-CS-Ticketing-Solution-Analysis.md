...
Also unchanged from v1.0: the task brief that commissioned this analysis referred to a **"Genesys Call Center"** integration, while the source PDF and both diagrams consistently and only use **"Geyness."** This remains unresolved — see **ISSUE-003**.
...
| **INT-02 Geyness/Genesys Call Center Platform** | Deliver phone/agent-desktop layer; hand off verified interactions. | Inbound | [ASSUMPTION] — depends on confirmed platform (**ISSUE-003**) | Manual entry fallback (already MVP's default mode) | Correlation ID linking call metadata to ticket | **Phase 2** |
...
- Genesys/Geyness call-center platform integration (INT-02) — **gated on ISSUE-003's vendor-identity decision**

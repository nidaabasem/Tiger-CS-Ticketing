...
| Department Employee | Own/assigned tickets | — | Own tickets | — | Request only | ✔ | **✔** (marks work done) | — *(not permitted — see ISSUE-022)* | — *(no direct Reopen; may request Reopen Approval in own department)* | ✔ | ✔ | — | — |
| Department Head | All dept tickets | — | All dept tickets | ✔ | ✔ (approve) | ✔ | ✔ | — *(not primary channel — see ISSUE-022)* | — *(no direct Reopen; may request Reopen Approval — see the Reopen note below)* | ✔ | ✔ | Dept reports | Dept config (routing rules) |
...
**Reopen column note (final approved rule):** Direct Reopen is the **CS layer only — CS Agent, CS Supervisor, CS Manager** — plus System Administrator through ADR-0024's central override (which is why the role is not named in `TicketRoleSets.Reopen`). Every other role, Department Head included, holds **no direct Reopen**: the action is not offered in the UI and the endpoint answers `403`. The Department Head cell above previously read "✔ (escalated cases)"; that is superseded here. Those roles instead go through **Reopen Approval**, documented immediately below.

### Reopen Approval — the request path (`ApprovalType.ReopenApproval`)

**Approval is not Reopen.** A Reopen Approval authorizes a *request*; the existing Reopen lifecycle (`TicketLifecycleAppService.ReopenAsync`) performs the actual state transition. Granting one changes **nothing** about the ticket — it stays `Closed`, in the same department, with the same owner, with no new SLA cycle and no customer email. Direct Reopen is unchanged and never requires an approval.

| | Who |
|---|---|
| **Direct Reopen** | CS Agent · CS Supervisor · CS Manager · System Administrator (ADR-0024 override) |
| **May request Reopen Approval** | Department Employee *(member of the ticket's current department)* · Department Head *(member of the ticket's current department)* · General Manager · Chairman/CEO |
| **Approver** | CS Manager *(configured as a Role-targeted `RequestTypeApprovalRequirement`)* · System Administrator (ADR-0024 override) |
| **Read only — neither path** | Reporting User |

Notes that matter:

- **Ownership is deliberately not part of the requester rule.** The ticket is Closed, so the work is finished; department membership decides who may ask for it back, not which employee happened to hold it last. This is the one place the approval subsystem's usual operational-actor rule (which has an owner branch) does not apply.
- **The roles that hold direct Reopen are never offered the request form** — they can simply reopen the ticket. The endpoint stays reachable for System Administrator, because ADR-0024 is an authorization override; the control is still suppressed, since the override already grants direct Reopen.
- **Configuration-driven, per request type.** A request type supports Reopen Approval only when an active `RequestTypeApprovalRequirement` configures it (Administration → Request Type → Approval requirements → Reopen Approval → Role → CS Manager). Nothing is seeded automatically.
- **Requestable on a Closed ticket, and only this type.** Accounting Approval and Customer Service Approval remain refused on a Closed ticket. The carve-out is additionally gated on the ticket being genuinely reopenable — Closed, closed as `Resolved`, `RequestType.AllowReopen`, and inside the ISSUE-011 window — so a request can never be raised for a reopen that could not happen.
- **Eligibility is re-checked at approval time.** If the window passed (or the configuration changed) while the cycle sat Pending, the approval is refused rather than granted; the cycle can still be *rejected*, so it is never stuck. There is no such thing as an Approved reopen request that cannot be executed.
- **The reopen reason is mandatory** and is stored in the approval's existing `RequestComment`, visible in the approvals panel, the activity feed and the audit trail.
- **The target department is not part of the request.** It is chosen by the CS user who performs the actual Reopen, together with the current `RowVersion` — and that execution revalidates Closed + Resolved, the window, `AllowReopen`, ticket access, the target department and concurrency. An approved request bypasses none of them.
- **No notifications.** As with the other approval types, the Pending Approval dashboard KPI and queue are the approver's surface; no approval email is sent, and the customer's Reopened email continues to come from the actual reopen only.
...
| TicketStatus | Closed → In Progress (Reopen event) | **CS Agent, CS Supervisor, CS Manager** (on customer contact); System Administrator via ADR-0024. No other role. | Within reopen window (**ISSUE-011**); `ReopenCount` incremented |
...
| BR-020 | **[Revised — approved Reopen business rule.]** A **closed** ticket, closed as `Resolved`, may be reopened by a CS-layer user — **CS Agent, CS Supervisor or CS Manager** (System Administrator through ADR-0024's override) — within the confirmed window (**ISSUE-011**), measured from closure. A Resolved ticket is not reopenable, and neither is one closed as Cancelled/Rejected/Duplicate. A role without direct Reopen — Department Employee or Department Head in the ticket's department, General Manager, Chairman/CEO — may instead raise a **Reopen Approval** request (`ApprovalType.ReopenApproval`, approved by CS Manager), which authorizes the ask but performs no part of the reopen: the transition still happens only through this rule, executed by a CS-layer user. Beyond the window the reopen is refused and a new ticket is raised for the customer instead — **the automatic creation of a *linked* successor ticket is still not built**, so that half of this rule remains an open design item, not implemented behaviour. | Approved rule (supersedes the original [ASSUMPTION]) |

...
| BR-020 | **[Revised — approved Reopen business rule.]** A **closed** ticket, closed as `Resolved`, may be reopened by a CS Agent within the confirmed window (**ISSUE-011**), measured from closure. A Resolved ticket is not reopenable, and neither is one closed as Cancelled/Rejected/Duplicate. Beyond the window the reopen is refused and a new ticket is raised for the customer instead — **the automatic creation of a *linked* successor ticket is still not built**, so that half of this rule remains an open design item, not implemented behaviour. | Approved rule (supersedes the original [ASSUMPTION]) |

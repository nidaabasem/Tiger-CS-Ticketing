...
| `GenesysInteractionEvents` (Finding DR-03) | ADR-0014 (idempotency), ADR-0019 | §6.1, §6.2, §6.4, §6.5 | 19, 20 | Two distinct events of the same type on one call are both retained, not collapsed (`MVP-Implementation-Backlog.md` W3-05's test requirements, reference plan). **Genesys Basic Integration is a core, committed feature in the 4-week/1-developer pilot, built against the confirmed contract and shipped behind a feature flag for operational safety (§0/§3 of that document) — see S-17/S-25 for the pilot plan's own idempotency test requirement, which uses the confirmed contract's own event identifier directly rather than the fallback composite key this row's reference-plan estimate assumed.** |

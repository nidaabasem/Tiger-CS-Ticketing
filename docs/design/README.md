...
**Status: Design for review — revised following a senior-.NET-solution-architect review pass, and twice more to record and then correct management's approved delivery decision.** This package refines the approved MVP architecture (`docs/architecture/`) to implementation-ready detail for a **4-week, 1-developer, AI-assisted functional pilot** (revised from the original 3-week/4-person plan — see `MVP-Implementation-Backlog.md` §0): entity/column detail, API contracts, UI wireframes, requirement traceability, an implementation backlog, a design review checklist, and a formal architecture-review findings log. **SLA due-date calculation, basic escalation, and Genesys Basic Integration (built against the confirmed Tiger/Genesys contract) are core, committed pilot features — Genesys ships behind a feature flag for operational safety, not because it is deferred. Priority downgrades are hard-disabled after ticket creation for the pilot; priority upgrades remain. No production deployment is authorized at this stage.**
...
5. **[Genesys Mock Contract](Genesys-Mock-Contract.md)** — the provisional, explicitly-not-official payload shape originally drafted behind the webhook endpoint in (4)'s §6. **The pilot's Genesys Basic Integration (`MVP-Implementation-Backlog.md` §3, S-17) is built against the confirmed Tiger/Genesys contract, not this mock** — this document remains as a reference for any future integration whose contract isn't yet confirmed
...
| `Genesys-Mock-Contract.md` | Provider-neutral, temporary webhook payload contract — explicitly not the official Genesys schema. **Superseded for this pilot by the confirmed Tiger/Genesys contract** (`MVP-Implementation-Backlog.md` §3); retained as a reference for any future unconfirmed integration |
| `Internal-CRM-API-Contract.md` | The exact, minimal read-only surface (3 operations, unit/contact lookup only) Tiger CS Ticketing's Customer Verification module requires from Tiger Group's in-house CRM — the consumer-side contract `ICrmGateway`/`MockCrmGateway` are built against. States the CRM/Ticketing ownership boundary explicitly: CRM is read-only data access only; verification decisions, state, and audit are Tiger CS Ticketing's own business logic |
...
| `MVP-Implementation-Backlog.md` | **Approved plan:** 4-week, 1-developer, AI-assisted sequential backlog (25 items + a named 12h reserve, 119h total against a 120h cap) with SLA/escalation/Genesys core and committed, priority downgrade hard-disabled, a workload-hours-per-week table, an optional-stretch-items list, a recalculated critical path, and a Pilot-Done vs. Production-Ready distinction (no production deployment authorized). **Reference appendix:** the original 3-week/4-person backlog, critical path, and parallel workstreams, retained for a future team scale-up |
...
See `MVP-Design-Review-Findings.md`'s "Remaining decision or dependency" column, `MVP-Traceability-Matrix.md` §9 (requirement-coverage gaps), and `MVP-Design-Review-Checklist.md`'s "Summary of Open Items Found During This Review" for the full list. Nothing there is silently resolved by this index — all three remain the authoritative open-items record. **DR-08 (backlog capacity/scope) is no longer an open item** — it was resolved by management's approved decision, corrected once so that SLA due-date calculation, basic escalation, and Genesys Basic Integration (against the confirmed contract, feature-flagged for operational safety) remain core and committed, priority downgrade is hard-disabled for the pilot rather than merely deferred, and no production deployment is authorized — recorded in `MVP-Implementation-Backlog.md` §0 and `MVP-Design-Review-Findings.md`. The items listed in `MVP-Implementation-Backlog.md` §2.6 (optional stretch, outside the 119h commitment) remain genuinely open for a future phase.

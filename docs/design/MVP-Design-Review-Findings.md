...
| **Date** | 2026-08-18; DR-08 updated twice following management's delivery decisions — first recording a 4-week/1-developer pilot, then corrected to reflect that SLA, escalation, and Genesys Basic Integration (against the confirmed contract) remain core and committed, with priority downgrade hard-disabled rather than merely deferred |
...
- **Remaining decision or dependency:** The 24-hour expiry window is `[ASSUMPTION]`, not confirmed. **Pilot-scope note (added when DR-08 was corrected):** this full request/approve/reject design is the approved **post-pilot** design and remains documented in full, unchanged, in `MVP-API-Contracts.md` §5.6.1–§5.6.5 and `MVP-ERD.md`/`MVP-Data-Dictionary.md` §2.27. For the pilot itself, management has decided to go further than deferring this workflow — **priority downgrades are hard-disabled entirely after ticket creation** during the pilot (`MVP-Implementation-Backlog.md` §0), which safely defers this workflow without permitting an unauthorized downgrade through any other path. See also `docs/architecture/adr/0012-priority-change-sla-policy.md` and `SLA-Architecture.md` §7 for the pilot-scope note added there.
...
### DR-08 — Backlog capacity gap (Architecture/Foundation over capacity even after rebalancing) — **RESOLVED by management decision, corrected once to match management's actual intent**

- **Severity:** High (planning risk, not a design defect).
- **Status:** **Resolved**, after one correction cycle. When first raised, this finding presented three options to the sponsor without adopting one. Management's first decision approved a 4-week/1-developer pilot but was implemented in this document by deferring SLA, escalation, and Genesys entirely — **management has since clarified that this over-deferred scope it did not intend.** The corrected, now-current decision (recorded in `MVP-Implementation-Backlog.md` §0): a 4-week, 1-developer, AI-assisted pilot in which **SLA due-date calculation, basic escalation, and Genesys Basic Integration (against the confirmed contract) are core and committed**, Genesys ships behind a feature flag for **operational safety, not scope exclusion**, priority downgrade is **hard-disabled** for the pilot (not merely deferred to an unbuilt approval workflow), and priority upgrade remains since it can be implemented safely. Mock validation is still never described as production-ready, and no production deployment is authorized at this stage.
- **Documents changed:** `MVP-Implementation-Backlog.md` (restructured a second time), `MVP-Design-Review-Checklist.md`, `MVP-Traceability-Matrix.md`, `MVP-UI-Wireframes.md`, `README.md`, `docs/architecture/adr/0012-priority-change-sla-policy.md`, `docs/architecture/SLA-Architecture.md`, this document.
- **Original finding (retained for the record):** Added a workload-summary table totaling every item's estimated effort per role per week — something the pre-review backlog never actually did despite asserting a 90-hour-per-role capacity. Found Architecture/Foundation's original 3-week/4-person total was already over capacity before this review.
- **First correction (superseded by the second, retained for the record):** A 1-developer/4-week plan that fit within 120 hours only by deferring SLA, escalation, and all Genesys work entirely. This did not match what management had actually approved.
- **Second correction (current):** `MVP-Implementation-Backlog.md` §2 now sequences 25 items (S-01–S-25) plus a 12-hour named reserve, totaling **119 ideal hours against a 120-hour cap** (1 hour of headroom, not a disclosed overage), with **no week exceeding 30 hours** — verified by independently recomputing every item's hours from the source text. §0.2 states exactly what stayed core (matching management's must-remain list item-for-item) and what was trimmed *within* each core feature (not removed) to make room — pause/resume, the timed Level 2→3 escalation auto-advance, the Genesys failed-events retry UI, attachment withdrawal, live admin screens, and the priority-downgrade workflow (hard-disabled, not built at all) are the trims, each named against management's own "may be deferred" list. §2.6 lists optional stretch items explicitly outside the 119h commitment, each with a rough added-hour estimate, so nothing is silently included or silently dropped. The original 4-person/3-week plan remains §5, unchanged, still marked superseded/reference-only.
- **Remaining decision or dependency:** None on the capacity or scope question — both are now decided and internally consistent. The stretch items in §2.6 remain real, undelivered functionality for a future phase, disclosed as such, not an open question needing another decision right now.
- **Implementation-blocking:** No — the corrected plan is internally consistent (every `S-##` dependency resolves; the workload table's hours were independently recomputed from the source items and match exactly) and ready to be worked from once implementation is authorized.
...
| DR-08 — Backlog capacity gap | High (planning) | **Resolved by management decision, corrected once** (4-week, 1-developer, AI-assisted pilot; SLA/escalation/Genesys core; downgrade hard-disabled) | No for code; the decision removes the "Yes" that previously applied to committing to an undecided timeline |

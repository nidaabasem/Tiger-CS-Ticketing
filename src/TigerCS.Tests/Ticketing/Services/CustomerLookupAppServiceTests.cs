...
/// <summary>
/// Business-rule change: searches CRM, PACT, and Tasleeh by phone number and
/// returns whatever each found — enrichment/identification only, never a
/// Ticket creation gate. Required regression coverage: each source's Found/
/// NotFound/Failed outcome, and that a partial result set (one source Found,
/// one Failed, one NotFound) still returns everything together.
/// </summary>
public class CustomerLookupAppServiceTests
{
    ...
    private static Fixture CreateService()
    {
        ...
        var service = new CustomerLookupAppService(intakeRecords, crmLookup, pact, tasleeh, crmUnitLookup);

        return new Fixture(service, intakeRecords, crmLookup, pact, tasleeh, crmGateway);
    }

    private static async Task<long> SeedIntakeAsync(FakeIntakeRecordRepository repo, string phoneNumber = Phone)
    {
        var record = new TigerCS.Domain.Modules.Ticketing.IntakeRecord(
            Channel.Phone, phoneNumber, isUnitRelated: false, rawUnitNumberEntered: null, priorityHint: null, Guid.NewGuid(), DateTime.UtcNow);
    ...
    [Fact]
    public async Task SearchAsync_AllThreeSourcesFindNothing_ReturnsNotFoundForEach()
    {
        var f = CreateService();
    ...
    [Fact]
    public async Task SearchAsync_PartialResults_CrmFoundPactFailedTasleehNotFound_ReturnsAllThreeTogether()
    {
        var f = CreateService();
        var intakeRecordId = await SeedIntakeAsync(f.IntakeRecords);
        var unit = new TigerCS.Application.Modules.CustomerVerification.CrmIntegration.CrmUnitResult("CRM-UNIT-1001", "1204", "Tiger Tower A", "Tower A", "Residential");

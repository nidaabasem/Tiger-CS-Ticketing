...
/// <summary>
/// Proves the CrmVerification policy (PolicyNames.CrmVerification) against
...
/// coding, not guessed. See PolicyNames.CrmVerification's own remarks.
/// </summary>
public class CrmVerificationAuthorizationTests : IClassFixture<TigerCsApiFactory>
{
    private readonly TigerCsApiFactory _factory;

    public CrmVerificationAuthorizationTests(TigerCsApiFactory factory) => _factory = factory;
    ...
    public async Task GetUnit_EveryRole_MatchesCrmVerificationPolicy(string role, bool expectAuthorized)
    {
    ...
    [Theory]
    [MemberData(nameof(AllRolesWithExpectedAccess))]
    public async Task GetContacts_EveryRole_MatchesCrmVerificationPolicy(string role, bool expectAuthorized)
    {
        var client = await CreateAuthenticatedClientAsync(role);
    ...
    [Theory]
    [MemberData(nameof(AllRolesWithExpectedAccess))]
    public async Task CreateVerificationSession_EveryRole_MatchesCrmVerificationPolicy(string role, bool expectAuthorized)
    {
        var client = await CreateAuthenticatedClientAsync(role);

...
/// unlike CustomerVerificationEndpointsTests' shared class fixture where other
/// tests already populate the cache first.
///
/// <para>
/// An honest, disclosed limitation, same shape as
/// CustomerVerificationEndpointsTests.CreateVerificationSession_ConcurrentDuplicateRequests_NeitherCallerCrashes:
...
/// do throw on a race, which CustomerVerificationUnitOfWork translates and
/// CrmUnitLookupAppService's UpsertUnitAsync/UpsertContactAsync recover
/// from — proven deterministically by
/// CrmUnitLookupAppServiceTests.GetUnitAsync_ConcurrentDuplicateWriteRace_ReturnsWinningRowInsteadOfThrowing
...
/// </para>
/// </summary>
public class CrmLookupUpsertConcurrencyTests : IClassFixture<TigerCsApiFactory>
{
    private readonly TigerCsApiFactory _factory;

    public CrmLookupUpsertConcurrencyTests(TigerCsApiFactory factory) => _factory = factory;

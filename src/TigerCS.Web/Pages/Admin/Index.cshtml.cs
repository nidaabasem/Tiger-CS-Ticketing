...
public sealed record AdminAreaCard(string Title, string Description, string Href, int? Count, string CountLabel);
    ...
    public async Task OnGetAsync(CancellationToken cancellationToken)
    {
        ...
        [
            new("Users", "Accounts, roles and department membership. Users are deactivated, never deleted.", "/Admin/Users",
                users.Result.IsSuccess ? users.Result.Value?.TotalCount : null, "active users"),
            new("Departments", "Responsible departments and their members.", "/Admin/Departments",
                departments.Result.IsSuccess ? departments.Result.Value?.Count : null, "active departments"),
            new("Request Types", "What each department handles, how it is assigned, what approvals and SLAs apply.", "/Admin/RequestTypes",
                requestTypes.Result.IsSuccess ? requestTypes.Result.Value?.Count : null, "active request types"),
            new("Workflows", "The versioned step sequences request types follow — Draft, Active, Historical.", "/Admin/Workflows",
                workflows.Result.IsSuccess ? workflows.Result.Value?.Count : null, "active workflows")
        ];
    }
}
